	...
	<div class="col-sm-12">
		<form class="well form-horizontal" id="Edit_Record" accept-charset="UTF-8" method="post">
			<div class="row">
				<div class="col-sm-12 edit-page-header">
							...
							<div class="published" {if $fields.listing_published eq 0}style="display: none;"{/if}>
								<!-- PUBLISHED -->
								<a href="javascript:void(0);" onClick="$('#Edit_Record').submit();" class="btn btn-primary pull-right top-btn published"><span class="glyphicon glyphicon-floppy-saved"></span> Save</a> <a href="javascript:void(0);"
									onClick="saveDraft('field[1][tbl_listing][{$cnt}][id]','listing_object_id','listing_published','field[1][tbl_listing][{$cnt}][listing_deleted]',false);" class="btn btn-info pull-right top-btn published"><span class="glyphicon glyphicon-floppy-disk"></span> Save Draft Version</a> <a href="javascript:void(0);"
									onClick="unpublish('listing_published');" class="btn btn-warning pull-right top-btn"><span class="glyphicon glyphicon-thumbs-down"></span> Unpublish</a>
							</div>
							<div class="drafts" {if $fields.listing_published eq 1}style="display: none;"{/if}>
								<!-- DRAFT -->
								<a href="javascript:void(0);" onClick="publish('field[1][tbl_listing][{$cnt}][id]','listing_object_id','listing_published','field[1][tbl_listing][{$cnt}][listing_deleted]');" class="btn btn-primary pull-right top-btn drafts"><span class="glyphicon glyphicon-thumbs-up"></span> Save &amp; Publish</a> <a href="javascript:void(0);"
									onClick="saveDraft('field[1][tbl_listing][{$cnt}][id]','listing_object_id','listing_published','field[1][tbl_listing][{$cnt}][listing_deleted]',false);" class="btn btn-info pull-right top-btn drafts"><span class="glyphicon glyphicon-floppy-saved"></span> Save</a>
							</div>
			...
			<ul class="nav nav-tabs" id="myTab">
				...
				<li><a href="#services" data-toggle="tab">Installation Services</a></li>
            ...
            <div class="row form-group">
              <hr/>
				...
				<!--===+++===+++===+++===+++===+++ LOCATION TAB +++===+++===+++===+++===+++====-->
				<div class="tab-pane form" id="location">
					<div class="row form-group">
				...
				<div class="tab-pane" id="services">
					<div class="row form" id="service-wrapper">{assign var='serviceno' value=0} {foreach $fields.services as $services} {assign var='serviceno' value=$serviceno+1} {include file='service.tpl'} {/foreach}</div>
					<div class="row btn-inform">
						<a href="javascript:void(0);" class="btn btn-success btn-add-new" onclick="$('.files').slideUp();newService();"> Add New Inst. Service</a>
					</div>
					<input type="hidden" value="{$serviceno}" id="serviceno">
				</div>
				<!--===+++===+++===+++===+++===+++ FILES TAB +++===+++===+++===+++===+++====-->
				...
				<div class="tab-pane" id="promotions">
					<div class="row form" id="promotions-wrapper">
						<div class="row form-group">
							<label class="col-sm-3 control-label" for="location_image">Promotion Image</label>
							<div class="col-sm-9">
								<input type="hidden" value="{$fields.location_image}" name="field[2][tbl_location][{$cnt}][location_image]" id="location_image_link" class="fileinput"> <span class="file-view" id="location_image_path"> {if $fields.location_image}<a href="{$fields.location_image}"
									target="_blank">View</a>{else}None{/if}
								</span> <a href="javascript:void(0);" class="btn btn-info marg-5r" onclick="getFileType('location_image','','');">Select File</a> <a href="javascript:void(0);" class="btn btn-info" onclick="$('#location_image_link').val('');$('#location_image_path').html('None');">Remove File</a>
							</div>
						</div>
						<div class="row form-group">
							<label class="col-sm-3 control-label" for="id_location_url">Promotion Video</label>
							<div class="col-sm-5">
								<input class="form-control" type="text" value="{$fields.location_url}" name="field[2][tbl_location][{$cnt}][location_url]" id="id_location_url">
							</div>
							<div class="col-sm-9 col-sm-offset-3">
								<small>https://www.youtube.com/watch?v=<b>[VIDEO_ID]</b></small>
							</div>
						</div>
					</div>
				</div>
...
function newService() {
	...
	$.ajax({
		...
		cache : false,
		data : "template=service.tpl&serviceno=" + no ,
		dataType : "html",
		success : function(data, textStatus) {
			try {
				$('#service-wrapper').append(data);
				$('body').css('cursor', 'default');
				scrolltodiv('#service_wrapper' + no);
			} catch (err) {
				$('body').css('cursor', 'default');
			}
		}
...
function deleteService(ID) {
	if (ConfirmDelete()) {
		...
		html = '<input type="hidden" value="'+today+'" name="field[10][tbl_service]['+count+'][service_deleted]" />';
		$('#' + ID).append(html);
		$('#' + ID).css('display', 'none');
