	...
	<form class="well form-horizontal" id="Edit_Record" accept-charset="UTF-8" action="/admin/includes/processes/processes-record.php" method="post">
		<div class="row-fluid">
			<div class="span12">
				...
				<input type="hidden" value="{$fields.category_id}" name="field[1][tbl_category][{$cnt}][category_id]" id="category_id" />
				<input type="hidden" value="category_id" name="field[1][tbl_category][{$cnt}][id]" id="id" />
				<input type="hidden" value="3" name="field[1][tbl_category][{$cnt}][category_type_id]" id="category_type_id" />
			</div>
		</div>
		 <div class="row-fluid control-group">
			<div class="span3"><label class="control-label" for="category_name">Name</label></div>
			<div class="span9 controls"><input type="text" value="{$fields.category_name}" name="field[1][tbl_category][{$cnt}][category_name]" id="category_name" ></div>
		</div>
		<div class="row-fluid control-group">
			<div class="span3"><label class="control-label" for="category_listing_id">Primary Page</label></div>
			<div class="span9 controls">
				<select name="field[1][tbl_category][{$cnt}][category_listing_id]" id="category_listing_id">
		...
		<div class="row-fluid control-group">
			...
			<div class="span9 controls">
				<select name="field[1][tbl_category][{$cnt}][category_parent_id]" id="category_parent_id">
				<option value="0">Select one</option>
		...
		<div class="row-fluid control-group">
			...
			<div class="span9 controls"><input type="text" value="{$fields.category_order}" name="field[1][tbl_category][{$cnt}][category_order]" id="category_order"></div>
		</div>
		<div class="row-fluid control-group">
			...
			<input type="hidden" value="{$type}" name="field[1][tbl_slide][{$cnt}][slide_type_id]"  />
		</div>
	</form>
	</div>
</div>
