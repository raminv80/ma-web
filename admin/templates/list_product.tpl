{block name=body} {* Define the function *} {function name=render_list level=0} {foreach from=$items key=key item=item}
<tr>
	{assign var=itemType value=$key|substr:0:1}
	<td {if $itemType == 'l'}style="color:#888888;"{/if}>{for $var=1 to $level}- {/for}<b>{$item.title}{if $itemType == 'l'}:{/if}</b></td>
