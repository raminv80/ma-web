...
if($res = $DBobject->wrappedSql($sql)){
	foreach($res as $r){
	  ...
	  foreach ($products as $product){
	    $Csql = "SELECT product_object_id FROM tbl_product 
	    WHERE (product_deleted IS NULL OR product_deleted = '0000-00-00') 
	    AND product_migration_id = :product_migration_id LIMIT 1";
	    if($Cres = $DBobject->wrappedSql($Csql, array('product_migration_id'=>trim($product)))){
	      ...
	      $checksql = "SELECT * FROM tbl_productassoc WHERE productassoc_product_id= :productassoc_product_id 
	          AND productassoc_product_object_id = :productassoc_product_object_id 
	          AND (productassoc_deleted IS NULL OR productassoc_deleted = '0000-00-00')";
