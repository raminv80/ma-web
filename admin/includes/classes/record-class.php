...
class Record{
  ...
  function getRecord($id){
    ...
    foreach($this->CONFIG->table->options->field as $f){
      ...
      if(!empty($article_f['options']["{$f->name}"])){
        foreach($f->associated as $a2){
          foreach($article_f['options']["{$f->name}"] as &$opt){
            $opt["{$a2->name}"] = $this->getAssociated($a2, $opt['id']);
          }
        }
      }
    }
  ...
  function getLog($a, $id){
    ...
    if((string)$a->id != (string)$a->field){
      ...
      $parent = $DBobject->wrappedSqlGet($sql, array(
          ':id' => $id 
      ));
