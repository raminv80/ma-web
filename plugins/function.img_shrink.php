 ...
 * Smarty plugin
 * -------------------------------------------------------------
 * File:	function.img_shrink.php
 * Type:	function
 * Name:	thumbnail
 * Purpose:	create shrink image version of the image
 * -------------------------------------------------------------
 */
include '/includes/plugins/crop.image.to.square.class.php';

function smarty_function_img_shrink($params, &$smarty)
{
	...
	try {
		...
		if( !file_exists("/images/medium/{$fname}") ){
			try{
				if (!file_exists("/images/thumbs")) {
					mkdir("/images/medium", 0777, true);
				}
