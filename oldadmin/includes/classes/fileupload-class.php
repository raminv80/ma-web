...
class uploader {
	...
	function upload($filename='', $accept_type='', $extention='') {
		...
		if (empty($_FILES) || !is_array($_FILES[$filename]) || !$_FILES[$filename]['name']) {
			$this->error = $this->get_error(0);
			$this->accepted  = FALSE;
			return FALSE;
		}
		// Get file extension from file name incase file type fails
		...
		if (empty($this->file['extention'])) $this->file['extention'] = "";
		if (empty($this->file['type']))      $this->file['type']      = "";
		if (empty($this->file['size']))      $this->file['size']      = "";
		if (empty($this->file['width']))     $this->file['width']     = "";
		if (empty($this->file['height']))    $this->file['height']    = "";
		if (empty($this->file['tmp_name']))  $this->file['tmp_name']  = "";
		if (empty($this->file['raw_name']))  $this->file['raw_name']  = "";
	 ...
	 *
	 */
	function save_file($path, $overwrite_mode="1",$generate_thumbnail=0){
		if ($this->error) {
			return false;
		}
		...
		if($this->accepted) {
			...
			switch((int) $overwrite_mode) {
				case 1: // overwrite mode
					...
					if(file_put_contents( $this->path . $this->file["name"],$f)){
						$success = true;
					} else {
						$success   =  false;
			...
			if($generate_thumbnail && $success) {
				// This is the temporary file created by PHP
				$uploadedfile = $this->path.$this->file["name"];

				// Create an Image from it so we can do the resize
				if(strtolower($this->file["extention"]) == '.gif' ) {
					$src = imagecreatefromgif($uploadedfile);
				}
				else if(strtolower($this->file["extention"]) == '.jpeg' ){
					$src = imagecreatefromjpeg($uploadedfile);
				}
				else if(strtolower($this->file["extention"]) == '.jpg' ){
					$src = imagecreatefromjpeg($uploadedfile);
				}
				else if(strtolower($this->file["extention"]) == '.png' ) {
					$src = imagecreatefrompng($uploadedfile);
				}
				else if(strtolower($this->file["extention"]) == '.bmp' ) {
					$src = imagecreatefromwbmp($uploadedfile);
				}
				else if(strtolower($this->file["extention"]) == '.xbm' ) {
					$src = imagecreatefromxbm($uploadedfile);
				}
				else if(strtolower($this->file["extention"]) == '.gd' ) {
					$src = imagecreatefromgd($uploadedfile);
				}
				else if(strtolower($this->file["extention"]) == '.gd2' ) {
					$src = imagecreatefromgd2($uploadedfile);
				}
				else {
					return (bool) $success;
				}
				...
				// if the image width and height is shit just copy the image for the thumb.
				if($width < 200) {
					$newwidth = $width;
					$newheight = $height;
				}
				else {
					$newwidth=$this->max_image_width;
				...
				$filename = $path."/thumb_". $this->file["name"];
				if(strtolower($this->file["extention"]) == '.gif' ) {
