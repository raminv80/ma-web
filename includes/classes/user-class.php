...
class UserClass{
  ...
  function Create($user, $overWriteExisting = false){
    if($res = $this->RetrieveByUsername($user['username'])){
      if($overWriteExisting){
        ...
        $this->user_id = 0;
        return array(
            'error' => "This email '{$user['username']}' has already been used." 
        );
      }
    } else{
  ...
  function UpdateDetails($data){
    ...
    return array(
        'error' => "This email '{$data['email']}' does not exist in our database." 
    );
  }
  ...
  function ResetPasswordToken($email){
    if($res = $this->RetrieveByUsername($email)){
      ...
      if($this->DBobj->wrappedSql($sql, $params)){
        return array(
            'success' => 'Your new password has been sent to the registered email address.', 
            'token' => $newPass, 
  ...
  /**
   * Reset the password field given the email, token and new password.
   * It returns associative array with user's details on success
   * On error return associative array: ('error')
   * Require strings: email and password
   *
   * @param string $email          
  ...
  function ResetPassword($email, $userToken, $newPassword){
    ...
    if($res = $this->RetrieveByUsername($email)){
      ...
      if($res2 = $this->DBobj->wrappedSql($sql, array(
          ':token' => $token 
      ))){
        if(empty($res2) || empty($res2[0]) || $res2[0]['expired'] == 1){
          ...
          if($this->DBobj->wrappedSql($sql, $params)){
            ...
            $this->DBobj->wrappedSql($sql, array(
                "id" => $res2[0]['user_id'] 
            ));
  ...
  function SetUnsubscribeToken($userId){
    ...
    if($res = $this->DBobj->wrappedSql($sql, array(
        ':user_id' => $userId 
    ))){
      ...
      $this->DBobj->wrappedSql($usql, array(
          "unsub" => $tok, 
          "id" => $userId 
      ));
      return true;
  ...
  function GetUsersAddresses($addressUserId = 0, $orderby = ''){
    ...
    return $this->DBobj->wrappedSql($sql, array(
        ':id' => $addressUserId 
    ));
  }
